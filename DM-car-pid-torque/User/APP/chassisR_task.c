...
float out_ang,out_vel,out_gyr;
static PID_t pid_ang = { .kp = 0.668f, .ki = 0.0f, .kd = 0.0469f, .integral = 0, .prev_err = 0 };  // 俯仰角误差
static PID_t pid_vel = { .kp = 0.053f, .ki = 0.0f, .kd = 0.33f, .integral = 0, .prev_err = 0 };  // v误差
static PID_t pid_gyr = { .kp = 0.015f, .ki = 0.0f, .kd = 0.0f, .integral = 0, .prev_err = 0 };  // 转向角误差
...
void ChassisR_task(void)
{
  ...
  ChassisR_init(&chassis_move);
	...
	{	
		...
		chassis_move.wheel_motor[0].wheel_T = out_ang + out_vel;
