...
uint8_t canx_send_data(FDCAN_HandleTypeDef *hcan, uint16_t id, uint8_t *data, uint32_t len)
{
  ...
  if(len<=8)	
	{
	  TxHeader.DataLength = len;     // 发送长度：8byte
	}
	else  if(len==12)	
	{
  ...
  if((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
  {
    if(hfdcan->Instance == FDCAN1)
    {
      ...
      HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &RxHeader1, g_Can1RxData);
			...
			{
        case 0x11 :dm6215_fbdata(&chassis_move.wheel_motor[0], g_Can1RxData,RxHeader1.DataLength);if(flag==1){a++;}break;
        case 0x12 :dm6215_fbdata(&chassis_move.wheel_motor[1], g_Can1RxData,RxHeader1.DataLength);if(flag==1){b++;}break;	         	
				default: break;
			}			
	  }
  }
